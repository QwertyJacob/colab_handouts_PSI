  ...
  "cells": [
    ...
    },
    {
      ...
      "source": [
        ...
        "\n",
        "> **In the following, we will place ourselves at time $t$. Suppose we are at time $t$. I.e. we observe $o_t$ as the result of $s_t$ (which is unknown), and we infer $s_t$ both as the origin of $o_t$ (posdiction) and as the result of $a_{t-1}$ (prediction, $a_{t-1}$ is known). We assume also that, at time $t$, $s_{t-1}$ is known.**\n",
        "\n",
        "For example:\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "$$\n",
        "\n",
        "The first lines in these boxes indicates that $\\hat{s}_{t}$ is a guess or inference about the real underlying state of the world, $s_{t}$, which is obtained as the output of neural networks, when these are fed with some inputs.\n",
        "\n",
        "Now the second lines are interesting because they are saying that we want to learn some optimal values of the nets parameters, $\\chi^*$ and $\\theta^*$, such that we can assume that its outputs are equivalent to _sampling_ from the distributions they approximate!"
      ]
    },
    {
      "cell_type": "markdown",
    ...
    },
    {
      "cell_type": "markdown",
      "source": [
        "Suppose for a second we are at time $t-2$, suppose we perform an action $a_{t-2}$, then we go to time $t-1$ and the state $s_{t-2}$ unhides. Then suppose we perform an action $a_{t-1}$, then we go to time $t$ and the state $s_{t-1}$ unhides.\n",
        "\n",
        "This means that, at (any) time $t$, we could use $o_{t-1}$ and $a_{t-2}$ to make a guess about $s_{t-1}$, say $\\hat{s}_{t-1}$, from the approx. posterior.\n",
        "\n",
        "This is done because we have the real $s_{t-1}$, and we can use that as the prior to compute the complexity term of the VFE as the L2 distance between $\\hat{s}_{t-1}$ and $s_{t-1}$:\n",
        "\n",
        " $$\n",
        "  \\begin{aligned}\n",
        "  &\\text{At (almost any) time $t$:}\\\\\n",
        "  \\tilde{s}_{t} &=  \\mathbf{q}_{\\theta}(o_{t}, a_{t-1})\\\\\n",
        "  \\text{VFE complexity term }& \\approx ||\\tilde{s}_{t-1} -s_{t-1} ||^2_2 \\\\\n",
        "  \\end{aligned}\n",
        "  $$\n",
        "\n",
        "Yes, I now, we cannot do that for every $t$ but only from $t=2$ (or $t=1$ if we create a phantom $a_{-1}$). But don't worry, from $t=2$ onwards, there's a lot of time for learning :)"
      ],
      "metadata": {
        "id": "OJ1Y46CVwCbj"
      },
    ...
    },
    {
      "cell_type": "markdown",
      "source": [
        "Now in this tutorial we could have used a \"likelihood model\" and optimise it using the accuracy VFE term. But we wont. We will focus on agency. Likelihood is going to be subsumed by our posterior (which can be thought of as an \"inverse likelihood\"), and we will minimise EFE (the whole formulation)."
      ],
      "metadata": {
        "id": "8ZM3R3sMZ5gh"
      },
    ...
    },
    {
      ...
      "source": [
        ...
        "\n",
        "We will use a neural network called the policy net, that is going to tell us each time what action to take :) This network is supposed to tell us what is the best action to take, i.e., the one that minimises the EFE functional.\n",
        "\n",
        "The policy network $\\mathbf{Q}_{\\phi}(s_t)$, will be parametrised by $\\phi$, and will output the actions $a_t$ given the current (hypothetical/unknown/inferred) state $s_t$. This is our policy model, our way of acting:\n",
        "\n",
        "$$\n",
        "\\boxed{\n",
        "\\begin{aligned}\n",
        ...
        "\\text{learn} \\quad \\phi^* \\quad \\text{such that}\\quad \\hat{a_t} \\sim \\sigma[-\\zeta \\mathbf{G}_{\\psi}(s_t, \\mathbf{a})]\\\\\n",
        "\\end{aligned}\n",
        "}\\tag{PN}\n",
        "$$\n",
        "\n",
        ...
        "\n",
        "- $\\sigma[\\cdot]$ is the Softmax operator, it basically transforms a *set* of numerical inputs into a proportionally equivalent probability distribution (i.e. a simplex).\n",
        "\n",
        "- $\\zeta$ is a precision hyperparameter. A scalar that shapes the actual polarisation or entropy of the resultant probability distribution function.\n",
        "\n",
        "- Finally, the $\\mathbf{G}_{\\psi}(s_t, \\mathbf{a})$ guy denotes a vector of the approximate EFE values for each possible action $a \\in \\mathbf{a}$. And yes, you might have guess right, that vector is the output of another neural network!"
      ],
      "metadata": {
        "id": "Nnmthf7oz864"
      },
    ...
    },
    {
      ...
      "source": [
        ...
        "\\underbrace{\\mathcal{G}(a_{t}, s_{t})}_{\\text{This is goundtruth EFE}} \\triangleq & \\underbrace{\\bar{\\mathcal{G}}(a_{t}, s_{t})}_{\\text{Real feedback}}  + \\underbrace{\\gamma}_{\\text{\"discount factor\"}} \\cdot \\max \\underbrace{ \\left[  \\hat{\\mathbf{G}}_{\\hat{\\psi}}(\\mathbf{a}, s_{t+1}) \\right]  }_{\\text{Critic's net EFE estimate }}\\\\\\\\\n",
    ...
    },
    {
      ...
      "source": [
        "It is not the case that we will implement the epistemic gain at time $t$, cuz we do not know the real posterior $Q(s_{t+1} | o_{t+1} , a_t)$ and that is precisely why we are making this tutorial. We actually do not even know the real posterior at time $t$, $Q(s_{t} | o_{t} , a_{t-1})$.\n",
        "\n",
        "\n",
        "However, at time $t$, $s_{t-1}$ is known and can be used as a point estimation of the real posterior, just as we did for the VFE.\n",
        "\n",
        "$$\n",
        "\\hat{s}_{t-1} =  \\mathbf{q}_{\\theta}(o_{t-1}, a_{t-2}) \\implies \\\\\n",
        "\\mathbb{E}_{Q( o_{t-1} | a_{t-2})}\\left[D_{KL}\\left[Q(s_{t-1} | o_{t-1} , a_{t-2}) ||q_{o_{t-1}}(s_{t-1}\\mid a_{t-2}) \\right]\\right] \\approx \\mathbb{E}_{Q( o_{t-1} | a_{t-2})}\\left[ \\left\\| {s}_{t-1} -  \\hat{s}_{t-1} \\right\\|_2^2\\right]$$\n",
        "\n",
        "Now if we actually see $o_{t-1}$, we can also approximate the outer expectation by the one sample:\n",
        "\n",
        "$$\\mathbb{E}_{Q( o_{t-1} | a_{t-2})}\\left[ \\left\\| {s}_{t-1} -  \\hat{s}_{t-1} \\right\\|_2^2\\right] \\approx \\frac12 \\left\\| {s}_{t-1} -  \\hat{s}_{t-1} \\right\\|_2^2$$\n",
        "\n",
        "Summarising:\n",
        "\n",
        "$$\n",
        "\\boxed{\n",
        "  \\begin{aligned}\n",
        "  &\\text{Approximated one-step epistemic gain (from $t-1$ to $t$):}\\\\\n",
        "  &\\hat{s}_{t-1} =  \\mathbf{q}_{\\theta}(o_{t-1}, a_{t-2})\\\\\n",
        "&D_{KL}\\left[Q(s_{t-1} | o_{t-1} , a_{t-2}) ||q_{o_{t-1}}(s_{t-1}\\mid a_{t-2}) \\right]\\approx \\frac{1}{2} \\left\\| {s}_{t-1} -  \\hat{s}_{t-1}\\right\\|_2^2\n",
        "\\end{aligned}\n",
        "}\n",
        "$$"
      ]
    ...
    },
    {
      "cell_type": "markdown",
      "source": [
        "This is our pragmatic fella: (one step future expectation)\n",
        "\n",
        "$$\n",
        "\\mathbb{E}_{Q( o_{t+1}| a_t)}\\left[ \\ln Q(o_{t+1} \\mid \\mathbf{C}) \\right]\n",
        "$$\n"
      ],
      "metadata": {
        "id": "X7bz0RWGxJei"
    ...
    },
    {
      ...
      "source": [
        "This time we could compute such a thing, cuz we're supposing to have taken $a_t$, and thus have seen $o_{t+1}$. However, it wouldn't be useful without its epistemic brother. That's why we use the pragmatic gain computation from $t-1$ to $t$ also...\n",
        "\n",
        "$$\n",
        "\\mathbb{E}_{Q( o_{t-1}| a_{t-2})}\\left[ \\ln Q(o_{t-1} \\mid \\mathbf{C}) \\right]\n",
        "$$\n",
        "\n",
        "Now the expectation is approximated by the corresponding observation sampled from the environment :\n",
        "$$\\mathbb{E}_{Q( o_{t-1}| a_{t-2})}\\left[ \\ln Q(o_{t-1} \\mid \\mathbf{C}) \\right]\n",
        " \\approx  \\ln Q(o_{t-1} \\mid \\mathbf{C})\n",
        "$$\n",
        "\n",
        "Now the normative observation is something we impose as a desideratum each time. Supposing we have a desired $o^*$, and assuming spherical Gaussian forms for the corresponding distributions, we'll have:\n",
        "\n",
        "$$\n",
        "\\boxed{\n",
        "  \\begin{aligned}\n",
    ...
    },
    {
      ...
      "source": [
        "So the one-step ground-truth for EFE will be:\n",
        "\n",
        "\n",
        "$$\n",
        "\\boxed{\n",
        ...
        "&\\bar{\\mathcal{G}}(a_t, s_t) \\approx - \\frac{1}{2} \\left\\| \\hat{s}_{t+1} -  \\bar{s}_{t+1}\\right\\|_2^2 - \\frac{1}{2} \\left\\| o_{t+1} - o_{t+1}^* \\right\\|_2^2\\\\\n",
        "\\end{aligned}\n",
        "}\n",
        "$$\n"
      ]
    ...
    },
    {
      "cell_type": "markdown",
      "source": [
        "Initialise your neural networks:\n",
        "\n",
        "$$\n",
        "\\begin{aligned}\n",
        "&\\text{1. Posterior Net:} \\quad \\mathbf{q}_{\\theta}\\leftarrow \\theta_{init}\\\\\n",
        "&\\text{2. Policy Net:} \\quad \\mathbf{Q}_{\\phi} \\leftarrow \\phi_{init}\\\\\n",
        "&\\text{3. Critic Net:} \\quad \\mathbf{G}_{\\psi} \\leftarrow \\psi_{init}\\\\\n",
        "&\\text{3(BIS). Target Critic Net:} \\quad \\hat{\\mathbf{G}}_{\\hat{\\psi}} \\leftarrow  \\mathbf{G}_{\\psi} \\quad \\text{(for training stability)}\\\\\n",
        "\\end{aligned}\n",
        "$$\n",
        "\n",
        "\n",
        ...
        "\n",
        "  - Infer $\\hat{s}_{t}$, using $o_{t}$ and $a_{t-1}$.\n",
        "\n",
        "  $$\\hat{s}_{t} = \\mathbf{q}_{\\theta}(o_{t},\\, a_{t-1})$$\n",
        "\n",
        "  - Choose a new action using your policy net and $\\hat{s}_{t}$:\n",
        "\n",
        "  $$\n",
        "  \\begin{aligned}\n",
        "  a_{t} &= \\mathbf{Q}_{\\phi}(\\hat{s}_{t})\\\\\n",
        "  \\end{aligned}\n",
        "  $$\n",
        "\n",
        "  - if $t>0$, 0ptimise posterior and policy nets:\n",
        "\n",
        "  $$\n",
        "  \\begin{aligned}\n",
        "  &\\text{Posterior (VFE complexity minimisation):}\\\\\n",
        "  \\tilde{s}_{t-1} &=  \\mathbf{q}_{\\theta}(o_{t-1}, a_{t-2})\\\\\n",
        "  \\theta &\\leftarrow \\theta -ϵ \\nabla\\left( ||\\tilde{s}_{t-1} -s_{t-1} ||^2_2\\right)\\\\\n",
        "  &\\text{Policy}\\\\\n",
        "  a_{t-1} &= \\mathbf{Q}_{\\phi}({s}_{t-1})\\\\\n",
        "  \\phi &\\leftarrow \\phi -ϵ \\nabla\\left( ||a_{t-1} - \\sigma[-\\zeta \\mathbf{G}_{\\psi}(\\mathbf{a}, s_{t-1})]||^2_2\\right)\\\\\n",
        "    &\\text{Critic (EFE minimisation):}\\\\\n",
        "  \\mathcal{G}(a_{t-2},s_{t-2})= &- \\frac{1}{2} \\left\\| s_{t-1} - \\hat{s}_{t-1} \\right\\|_2^2 - \\frac{1}{2} \\left\\| o_{t-1} - o^* \\right\\|_2^2 + \\gamma \\, \\hat{\\mathbf{G}}_{\\hat{\\psi}}(a_{t-1}, s_{t-1})\\\\\n",
        "  \\psi &\\leftarrow \\psi -ϵ \\nabla\\left( ||\\mathbf{G}_{\\psi}(a_{t-2}, s_{t-2}) - \\mathcal{G}(a_{t-2},s_{t-2})||^2_2\\right)\\\\\n",
        "  \\end{aligned}\n",
        "  $$\n",
        "\n",
        "\n",
        "  - Adjust your critic's target each $M$ steps:\n",
        "  $$\n",
        "  \\begin{aligned}\n",
        ...
        "\n",
        "\n",
        "  Note: $ \\hat{\\mathbf{G}}_{\\hat{\\psi}}(a_{t-1}, s_{t-1})$ states for the $a_{t-1}$'s corresponding EFE value from those outputted by $ \\hat{\\mathbf{G}}_{\\hat{\\psi}}(\\mathbf{a}, s_{t-1})$. Similarly, $\\mathbf{G}_{\\psi}(a_{t-2}, s_{t-2})$ indicates that we do   $ \\hat{\\mathbf{G}}_{\\hat{\\psi}}(\\mathbf{a}, s_{t-2})$ and from the resultant vector we take the component that corresponds to $a_{t-2}$.\n",
        "\n",
        "  Yes, this tutorial uses discrete action spaces. Give me some time, I'll make some stuff for continuous action spaces..."
      ],
      "metadata": {
        "id": "Gw9KbxwzLqXR"
      },
